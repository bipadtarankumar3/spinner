            ...
            <div class="col-xl-5 col-xxl-4">
              <div class="authentication-login min-vh-100 bg-body row justify-content-center align-items-center p-4">
                <div class="col-sm-8 col-md-6 col-xl-9">
                  ...
                  <p class=" mb-9">Update Your Password</p>

                  <h6 class="card-subtitle line-on-side text-muted text-center font-small-3 pt-2">
                    @if ($adminStatus == 'urlAvailable')
                        <span>Confirm Your Password</span>
                    @else
                        <span>Url Is Not Available</span>
                    @endif
                  </h6>
                  @if ($adminStatus == 'urlAvailable')
                  <form method="POST" action="{{URL::to('confirmPassword')}}">
                    @csrf
                    <input type="hidden" name="adminKey" value="{{$adminKey}}">
                    <div class="mb-3">
                      <label for="exampleInputEmail1" class="form-label">Password</label>
                      <input type="password" name="password" class="form-control" id="exampleInputEmail1" placeholder="Enter Password" aria-describedby="emailHelp" required>
                    </div>
                    <div class="mb-3">
                      <label for="confirmPassword" class="form-label">Confirm Password</label>
                      <input type="password" name="confirmPassword" class="form-control" id="confirmPassword" placeholder="Enter Confirm Password" aria-describedby="emailHelp" required>
                    </div>
